...
interface MacrosPanelProps {
  ...
  // Commit-free solve preview (see page.tsx) painted via direct DOM mid-drag.
  onPreview?: (macro: keyof MacroRatios, target: number) => MacroRatios;
  onRecalibrate?: () => void;
...
export function MacrosPanel({
  ...
  const applyDragVisual = useCallback((d: { key: MacroKey; pos: number; value: number }) => {
    const fill = fillRefs.current.get(d.key);
    if (fill) fill.style.width = `${(d.pos / SLIDER_SCALE) * 100}%`;
  ...
  const applySliderDom = useCallback(
    (key: MacroKey, r: MacroRatios) => {
      ...
      if (fill) fill.style.width = `${g.fillPct}%`;
  ...
  const previewTick = useCallback(() => {
    ...
    const r = onPreview(d.key, d.value);
    // CUP ONLY. Updating the five sibling sliders here (el.value on native
    // range controls) re-suppressed the gesture even from rAF (v15: ~38
    // events) — native form-control repaint is expensive in WebKit and lands
    // in the same round trip the mouse events wait on. The cup's SVG region
    // is measured-affordable per frame (its waterline already animates on
    // rAF with zero suppression). Siblings catch up on pause/release.
  ...
  const startPreviewLoop = useCallback(() => {
    if (previewRafRef.current !== null || !onPreview) return;
    const loop = () => {
      previewRafRef.current = requestAnimationFrame(loop);
      const now = performance.now();
      if (now - lastPreviewTimeRef.current < PREVIEW_MS) return;
  ...
  const scheduleSolve = useCallback(
    (macro: keyof MacroRatios, target: number) => {
      ...
      // Trailing-only: every event pushes the commit out past the quiet window.
